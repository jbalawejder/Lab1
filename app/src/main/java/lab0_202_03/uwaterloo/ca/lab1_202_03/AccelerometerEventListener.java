...
public class AccelerometerEventListener implements SensorEventListener{
    ...
    public void onSensorChanged(SensorEvent event){ // SensorEvent represents a single message from a single sensor
        if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            graph.addPoint(event.values); // Add the current x, y, and z component readings to the graph
            ...
            // Output maximum and current values for all components of the accelerometer
            output.setText(String.format("------ Maximum Accelerometer Reading ------\n" +
                    "X: %f\n" +
                    "Y: %f\n" +
                    "Z: %f\n\n" +
